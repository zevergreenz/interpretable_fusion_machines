...
decoder_upsample = Dense(filters * 16 *
                         16, activation='relu')
...
vae.save('cifar10_ld_%d_conv_%d_id_%d_e_%d_vae.h5' %
         (latent_dim, num_conv, intermediate_dim, epochs))
encoder.save('cifar10_ld_%d_conv_%d_id_%d_e_%d_encoder.h5' %
             (latent_dim, num_conv, intermediate_dim, epochs))
generator.save('cifar10_ld_%d_conv_%d_id_%d_e_%d_generator.h5' %
               (latent_dim, num_conv, intermediate_dim, epochs))

# save training history
# fname = 'cifar10_ld_%d_conv_%d_id_%d_e_%d_history.pkl' % (
#     latent_dim, num_conv, intermediate_dim, epochs)
...
for i in range(10):
    ...
    plt.savefig('%d.png' % i)
    plt.clf()
    plt.imshow(generator.predict(encoder.predict(x_train[i:i+1, :, :, :]))[0, :, :, :])
